<?php
/**
 * The MetaModels extension allows the creation of multiple collections of custom items,
 * each with its own unique set of selectable attributes, with attribute extendability.
 * The Front-End modules allow you to build powerful listing and filtering of the
 * data in each collection.
 *
 * PHP version 5
 * @package	   MetaModels
 * @subpackage AttributeTranslatedTags
 * @author     Christian Schiffler <c.schiffler@cyberspectrum.de>
 * @copyright  CyberSpectrum
 * @license    private
 * @filesource
 */
if (!defined('TL_ROOT'))
{
	die('You cannot access this file directly!');
}

/**
 * This is the MetaModelFilterRule class for handling select fields.
 * 
 * @package	   MetaModels
 * @subpackage AttributeTranslatedTags
 * @author     Christian Schiffler <c.schiffler@cyberspectrum.de>
 */
class MetaModelFilterRuleTranslatedTags extends MetaModelFilterRule
{

	public function __construct(MetaModelAttributeTranslatedTags $objAttribute, $strValue)
	{
		parent::__construct($objAttribute);
		$this->value = $strValue;
	}

	public function sanitizeValue()
	{
		$strTableNameId = $this->objAttribute->get('tag_table');
		$strColNameId = $this->objAttribute->get('tag_id');
		$strColNameAlias = $this->objAttribute->get('tag_alias');
		
		$arrValues = explode(',', $this->value);

		$objDB = Database::getInstance();

		if ($strColNameAlias)
		{
			$arrLookup = array_map('mysql_real_escape_string', $arrValues);
			$objSelectIds = $objDB->execute('SELECT ' . $strColNameId . ' FROM ' . $strTableNameId . ' WHERE ' . $strColNameAlias . ' IN (\'' . implode('\',\'', $arrLookup) . '\')');
			FB::log($objSelectIds, '$objSelectIds');
			$arrValues = $objSelectIds->fetchEach($strColNameId);
		} else {
			$arrValues = array_map('intval', $arrValues);
		}
		return $arrValues;
	}

	/**
	 * {@inheritdoc}
	 */
	public function getMatchingIds()
	{
		$arrValues = $this->sanitizeValue();
		FB::log($arrValues, 'sanitized Values translatedTags');
		if (count($arrValues) == 0) return array();
		$objDB = Database::getInstance();
		FB::log($objDB);
		$objMatches = $objDB->prepare('SELECT item_id as id FROM tl_metamodel_tag_relation WHERE value_id IN (' . implode(',', $arrValues) . ') AND att_id = ?')
				->execute($this->objAttribute->get('id'));
		FB::log($objMatches->fetchEach('id'), 'matching ids');
		return $objMatches->fetchEach('id');
	}
}

?>